...
#include "quintic_spline_interpolation.h"

namespace math {

// Interpolates the SO3 rotation by a quintic spline.
template <typename T>
class QuinticSplineInterpolationSO3 {
 public:
  ...
  QuinticSplineInterpolationSO3(const Knot& start, const Knot& end)
    ...
    spline_ = QuinticSplineInterpolation<Eigen::Vector3<T>>({start.t, zero, start.yd, start.ydd},
                                                            {end.t, end_delta_y_, end.yd, end.ydd});
  }
  QuinticSplineInterpolationSO3() = default;
 ...
 private:
  ...
  QuinticSplineInterpolation<Eigen::Vector3<T>> spline_;
};
}  // namespace math
